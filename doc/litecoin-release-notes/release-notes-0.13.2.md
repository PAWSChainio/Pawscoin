Pawscoin Core version 0.13.2 is now available from:

  <https://download.pawschain.io/litecoin-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/PAWSChainio/Pawscoin/issues>
...
ECDSA signatures inside Pawscoin transactions now use validation using
...
Previous versions of Pawscoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Pawscoin Core was to raise the default minimum
relay fee.

Pawscoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Pawscoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running pawscoind without having to do any manual configuration.
...
Pawscoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Pawscoin Core will relay transactions with
...
entirely in Pawscoin Core 0.13, and it is at this time undecided whether the
...
Pawscoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Pawscoin Core automatically creates a hidden service to listen on, without
manual configuration. Pawscoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Pawscoin Core is listening, and
...
Pawscoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Pawscoin
Core to use floating fees. By default, Pawscoin Core will use floating
fees.
...
At all times, Pawscoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) LTC.
Furthermore, Pawscoin Core will never create transactions paying less than
...
`pawscoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /litecoinrpc>
        ...
        # optional bypass pawscoind rpc basic auth
...
Restarting pawscoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in pawscoin.conf
...
pawscoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/pawscoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Pawscoin Core code base has
...
for a new opcode in the Pawscoin scripting system that in combination with
...
Pawscoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Pawscoin Core.
...
The two phases are distinct in the Pawscoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Pawscoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Pawscoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Pawscoin block need to be stored by nodes in order to validate future
...
- **Increased security for multisig:** Pawscoin addresses (both P2PKH addresses
  ...
  believed to be provided by Pawscoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Pawscoin's current ledger (called the UTXO set),
  ...
  note that Pawscoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Pawscoin users to individually opt-in to almost any change in the Pawscoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Pawscoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Pawscoin
client to Pawscoin Core 0.13.2 to signal for segwit support. When a
...
Since Pawscoin Core 0.10.0, nodes have defaulted to only relaying and
